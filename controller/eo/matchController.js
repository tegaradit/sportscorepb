...
const { getMatchDetailData } = require('../../helpers/getMatchDetailData'); // Pastikan ini ada
...
exports.manualCreateBracket = async (req, res) => {
  try {
    const {
      ...
      waktu,
      kode,
      jumlah_babak, // Ini adalah jumlah babak total untuk kategori, bukan per match
      durasi_babak    // Ini adalah durasi per babak untuk kategori, bukan per match
    } = req.body;
    ...
    // Ambil durasi_babak dan jumlah_babak dari event_categories
    // Karena kolom ini ada di event_categories dan matches
    const [[kategoriSetting]] = await pool.query(
        `SELECT durasi_babak, jumlah_babak FROM event_categories WHERE id = ?`,
        [id_kategori]
    );
    ...
    const final_durasi_babak = durasi_babak || kategoriSetting.durasi_babak;
    const final_jumlah_babak = jumlah_babak || kategoriSetting.jumlah_babak;


    // 1️⃣ Buat match terlebih dahulu
    // PERBAIKI: Hapus koma berlebih setelah 'waktu', tambahkan urutan kolom yang benar
    // sesuai dengan urutan VALUES yang akan Anda berikan.
    // Kolom 'status', 'skor_1', 'skor_2', 'adu_pinalti' memiliki DEFAULT VALUE, tidak perlu dimasukkan di INSERT jika menggunakan default.
    // Kolom `babak` juga punya default '1', bisa diabaikan kalau mau pakai default.
    const [matchInsert] = await pool.query(`
      INSERT INTO matches (
        id_kategori, team_1, team_2, waktu, status, skor_1, skor_2, adu_pinalti, babak, jumlah_babak, durasi_babak
      ) VALUES (?, ?, ?, ?, 'belum_main', 0, 0, 0, 1, ?, ?)
    `, [id_kategori, team_1, team_2, waktu_full, final_jumlah_babak, final_durasi_babak]);

    const match_id = matchInsert.insertId;

    // 2️⃣ Buat bracket dan sambungkan ke match_id
    // PERBAIKI: Kolom 'status', 'skor_1', 'skor_2', 'adu_pinalti' memiliki DEFAULT VALUE
    // Anda sudah memasukkan `kode` di akhir VALUES, itu benar.
...
exports.getBrackets = async (req, res) => {
  try {
    const { id_kategori, id_event } = req.query; // Bisa filter berdasarkan kategori atau event
    const todayOnly = req.query.today === 'true'; // Opsi untuk hanya mengambil yang hari ini

    if (!id_kategori && !id_event) {
      return res.status(400).json({ message: "Parameter id_kategori atau id_event harus disediakan." });
    }
    ...
    let queryBracket = `
      SELECT
        ...
        b.status,
        b.kode,
        b.jumlah_babak,  -- Pastikan ini diambil
        b.durasi_babak,  -- Pastikan ini diambil
    ...
    if (id_kategori) {
      queryConditions.push(`b.id_kategori = ?`);
      queryParams.push(id_kategori);
    } else if (id_event) {
      queryConditions.push(`ec.id_event = ?`);
      queryParams.push(id_event);
    }
    ...
    if (todayOnly) {
      ...
      queryConditions.push(`b.waktu BETWEEN ? AND ?`);
      queryParams.push(todayStart, todayEnd);
    }
    
    if (queryConditions.length > 0) {
      queryBracket += ` WHERE ` + queryConditions.join(' AND ');
    }

    queryBracket += ` ORDER BY b.waktu`;

    const [brackets] = await pool.query(queryBracket, queryParams);

    res.status(200).json(brackets); // Langsung kirim array brackets

  } catch (error) {
    console.error("❌ Error getBrackets:", error);
    res.status(500).json({ message: "Gagal mengambil pertandingan bracket" });
  }
};
...
exports.addMatchEvent = async (req, res) => {
  try {
    ...
    const matchId = parseInt(id_match);
    const menitInt = parseInt(menit);

    // ✅ Validasi dasar
    if (isNaN(matchId) || isNaN(menitInt)) {
      return res.status(400).json({ message: 'ID match dan menit harus berupa angka' });
    }
    ...
    // ✅ Simpan ke tabel match_events
    await pool.query(`
      INSERT INTO match_events (id_match, id_kategori, id_team, id_pemain, jenis, menit)
      VALUES (?, ?, ?, ?, ?, ?)
    `, [matchId, id_kategori, id_team, id_pemain, jenis, menitInt]);

    // ✅ Ambil data skor lama
    ...
    // ✅ Update skor tergantung jenis aksi
    if (jenis === 'GOL') {
    ...
    await pool.query(`
      ...
      SET skor_1 = ?, skor_2 = ?
      WHERE id = ?
    `, [team_1_score, team_2_score, matchId]);

    // ✅ Update atau insert ke pemain_event jika jenis GOL
    if (jenis === 'GOL') {
      ...
      if (cek.length > 0) {
        ...
        // ✅ Ambil nomor punggung dari lineup
        ...
        `, [matchId, id_pemain, id_team, id_kategori]);
    ...
    // ✅ Ambil detail terbaru untuk diemit ke client
    const fullMatchDetail = await getMatchDetailData(matchId);

    // ✅ Emit ke semua client di room pertandingan
    io.to(`match-${matchId}`).emit(`match:${matchId}:detail_updated`, fullMatchDetail);
    console.log(`[Socket] Emit: match:${matchId}:detail_updated`);

    // ✅ Response ke client HTTP
